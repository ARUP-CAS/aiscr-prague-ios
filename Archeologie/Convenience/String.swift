...
extension String {
  ...
  func decodingHTMLEntities() -> String {
    ...
    var htmlToAttributedString: NSAttributedString? {
       return self.htmlToAttributedString(css: "")
    }
    var htmlToAttributedDefault: NSAttributedString? {
        return self.htmlToAttributedString(css: "body,p {font-family:'Gotham CE';font-size:14px;} h4 {font-family:'Gotham-Bold';}")
    }
    
    func htmlToAttributedString(css style:String) -> NSAttributedString? {
        guard let data = "<html><head><style>\(style)</style></head><body>\(self)</body></html>".data(using: .utf8) else { return NSAttributedString() }
        do {
            let attr = try NSAttributedString(data: data, options:     [NSAttributedString.DocumentReadingOptionKey.documentType:  NSAttributedString.DocumentType.html,NSAttributedString.DocumentReadingOptionKey.characterEncoding:String.Encoding.utf8.rawValue], documentAttributes: nil)
            return attr
        } catch {
